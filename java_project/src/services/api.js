...
export const fetchHomeDataApi = async () => {
  ...
  const normalizeList = (items) =>
    normalizeProductsResponse({ products: { content: items ?? [] } }).products?.content ?? [];

  return {
    ...data,
    bestSellers: normalizeList(data.bestSellers),
    featured: normalizeList(data.featured),
  };
};
...
export const resumeSePayPaymentApi = async (orderId) => {
  const response = await clientAuthApi.post(`/checkout/sepay/pay/${orderId}`);
  return response.data;
};

export const syncSePayPaymentApi = async (orderId) => {
  const response = await clientAuthApi.post(`/checkout/sepay/sync/${orderId}`);
  return response.data;
};

export const fetchPendingPaymentOrdersApi = async () => {
  const response = await clientAuthApi.get("/orders/pending-payment");
  return response.data;
};