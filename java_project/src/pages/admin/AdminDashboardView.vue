...
const loadDashboard = async () => {
  ...
  try {
    ...
    const [data, rev] = await Promise.all([
      ...
      apiFetch(`/api/admin/reports/summary?from=${from}&to=${to}`).catch((e) => {
        console.warn("Revenue summary unavailable", e);
        return null;
      }),
    ]);
    stats.value = {
    ...
    revenueSummary.value = rev && rev.totalRevenue != null ? rev : {};
  } catch (e) {
