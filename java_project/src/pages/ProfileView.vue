        ...
        <div class="profile-layout">
          ...
          <div class="profile-content">
            ...
            <section v-show="activeTab === 'orders'" class="profile-panel">
              ...
              <p class="profile-panel__desc">Theo dõi trạng thái, thanh toán lại đơn SePay chưa hoàn tất và xem hóa đơn.</p>

              <div v-if="pendingOrders.length" class="profile-pending-pay">
                <h3>Đơn chờ thanh toán SePay</h3>
                <p class="profile-pending-pay__hint">
                  Các đơn bạn đã đặt nhưng chưa hoàn tất chuyển khoản. Bấm "Thanh toán ngay" để tiếp tục.
                </p>
                <article
                  v-for="order in pendingOrders"
                  :key="'pending-' + order.id"
                  class="profile-pending-pay__card"
                >
                  <div>
                    <strong>Đơn #{{ order.id }}</strong>
                    <span class="profile-pending-pay__amount">{{ formatPrice(order.totalAmount) }}đ</span>
                    <p class="profile-pending-pay__date">{{ formatOrderDate(order.createDate) }}</p>
                  </div>
                  <button
                    type="button"
                    class="profile-pending-pay__btn profile-pending-pay__btn--outline"
                    :disabled="checkingOrderId === order.id"
                    @click="checkSePayPayment(order.id)"
                  >
                    {{ checkingOrderId === order.id ? "Đang kiểm tra..." : "Đã chuyển — kiểm tra" }}
                  </button>
                  <button
                    type="button"
                    class="profile-pending-pay__btn"
                    :disabled="payingOrderId === order.id"
                    @click="paySePayOrder(order.id)"
                  >
              ...
              <div v-else class="profile-orders">
                ...
                >
                  ...
                  <div class="profile-order-card__actions">
                    <button
                      v-if="order.canPaySePay"
                      type="button"
                      class="profile-order-pay-btn profile-order-pay-btn--outline"
                      :disabled="checkingOrderId === order.id"
                      @click="checkSePayPayment(order.id)"
                    >
                      {{ checkingOrderId === order.id ? "Đang kiểm tra..." : "Kiểm tra TT" }}
                    </button>
                    <button
                      v-if="order.canPaySePay"
                      type="button"
                      ...
                      @click="paySePayOrder(order.id)"
                    >
                      {{ payingOrderId === order.id ? "Đang chuyển..." : "Thanh toán SePay" }}
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      {{ expandedOrderId === order.id ? "Thu gọn" : "Xem chi tiết / hóa đơn" }}
                    </button>
                  </div>
                  ...
                  <div v-if="expandedOrderId === order.id" class="profile-order-detail">
                    ...
                    <template v-else-if="orderDetail">
                      <div class="profile-invoice">
                        <h4>Hóa đơn {{ orderDetail.invoiceCode || ('#' + orderDetail.id) }}</h4>
                        <div class="profile-invoice__grid">
                          <span>Mã SePay</span><strong>{{ orderDetail.sepayInvoiceNumber || "—" }}</strong>
                          <span>Trạng thái TT</span><strong>{{ paymentStatusLabel(orderDetail.paymentStatus) }}</strong>
                          <span>Tạm tính</span><strong>{{ formatPrice(orderDetail.subTotal) }}đ</strong>
                          <span>Giảm giá</span><strong>-{{ formatPrice(orderDetail.discountAmount) }}đ</strong>
                          <span>Tổng thanh toán</span><strong class="profile-invoice__total">{{ formatPrice(orderDetail.totalAmount) }}đ</strong>
                        </div>
                        <p v-if="orderDetail.paymentStatus === 'PAID'" class="profile-invoice__paid">
                          ✓ Đã xác nhận thanh toán đủ {{ formatPrice(orderDetail.totalAmount) }}đ
                        </p>
                        <p v-else-if="orderDetail.canPaySePay" class="profile-invoice__unpaid">
                          Chưa nhận xác nhận chuyển khoản đủ số tiền từ SePay.
                        </p>
                      </div>
...
const pendingOrders = computed(() =>
  myOrders.value.filter((order) => order.canPaySePay && order.paymentStatus !== "PAID")
);
...
const paySePayOrder = async (orderId) => {
  payingOrderId.value = orderId;
  try {
    const data = await resumeSePayPaymentApi(orderId);
    if (data?.sepay) {
      submitSePayForm(data.sepay);
      return;
    }
    toast.error("Không lấy được liên kết thanh toán SePay.");
  } catch (err) {
    toast.error(err?.response?.data?.message || err?.message || "Không thể mở cổng thanh toán.");
  } finally {
    payingOrderId.value = null;
  }
};

const checkSePayPayment = async (orderId) => {
  checkingOrderId.value = orderId;
  try {
    const data = await syncSePayPaymentApi(orderId);
    ...
    const msg = data?.sepaySync?.message || data?.message || "SePay chưa xác nhận thanh toán.";
    toast.warning(msg);
  } catch (err) {
    toast.error(err?.response?.data?.message || "Không kiểm tra được trạng thái.");
  } finally {
...
const openOrderFromQuery = async () => {
  const rawId = route.query.orderId;
  if (!rawId) return;
...
watch(
  () => route.query.orderId,
  () => {
    if (activeTab.value === "orders" && myOrders.value.length) {
      openOrderFromQuery();
    }
  }
);
