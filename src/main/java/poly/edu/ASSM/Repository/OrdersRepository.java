...
public interface OrdersRepository extends JpaRepository<Orders, Integer> {
    ...
    @Query("""
            SELECT COUNT(o)
            FROM Orders o
            WHERE o.orderStatus IS NULL OR UPPER(o.orderStatus) <> 'CANCELLED'
            """)
    ...
    @Query("""
            ...
            FROM Orders o
            WHERE (o.orderStatus IS NULL OR UPPER(o.orderStatus) <> 'CANCELLED')
              AND (o.createDate IS NULL OR (o.createDate >= :start AND o.createDate < :end))
            """)
    long countActiveInRange(@Param("start") Instant start, @Param("end") Instant end);
    ...
    @Query("""
            ...
            WHERE (o.orderStatus IS NULL OR UPPER(o.orderStatus) <> 'CANCELLED')
              AND (o.createDate IS NULL OR (o.createDate >= :start AND o.createDate < :end))
            """)
    List<Orders> findActiveInRange(@Param("start") Instant start, @Param("end") Instant end);
    ...
    @Query("""
            SELECT o
            FROM Orders o
            WHERE UPPER(COALESCE(o.paymentStatus, 'UNPAID')) <> 'PAID'
              AND (o.paymentMethod IS NULL OR UPPER(o.paymentMethod) = 'SEPAY')
              AND (o.orderStatus IS NULL OR UPPER(o.orderStatus) <> 'CANCELLED')
              AND o.createDate >= :since
            ORDER BY o.createDate DESC
            """)
    List<Orders> findRecentUnpaidSePay(@Param("since") Instant since);
}
